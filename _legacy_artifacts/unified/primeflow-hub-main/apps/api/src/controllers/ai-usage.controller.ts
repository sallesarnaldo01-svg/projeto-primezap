...
export const aiUsageController = {
  ...
  async list(req: Request, res: Response) {
    try {
      ...
      logger.error({ error }, 'Erro ao listar uso de IA');
  ...
  async byLead(req: Request, res: Response) {
    try {
      ...
      logger.error({ error }, 'Erro ao obter uso por lead');
      throw error;
    }
  },
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      logger.info({ usageId: usage.id, model, cost }, 'Uso de IA registrado');
      res.status(201).json(usage);
    } catch (error) {
      logger.error({ error }, 'Erro ao registrar uso');
      throw error;
    }
  }
};
