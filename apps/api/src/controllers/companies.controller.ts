...
function mapCompany(company: CompanyWithRelations) {
  return {
    ...
    owner: company.owner
      ...
      : undefined,
    lastInteraction: company.lastInteractionAt?.toISOString(),
    customFields: (company.customFields as Record<string, unknown> | undefined) ?? {},
...
export async function createCompany(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const ownerData: { ownerId?: string } = {};
    if (payload.ownerId) {
      const owner = await prisma.public_users.findFirst({
        where: { id: payload.ownerId, tenantId: auth.tenantId },
      });
...
export async function updateCompany(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const ownerData: { ownerId?: string | null } = {};
    if (payload.ownerId !== undefined) {
      if (payload.ownerId === null) {
        ownerData.ownerId = null;
      } else {
        const owner = await prisma.public_users.findFirst({
          where: { id: payload.ownerId, tenantId: auth.tenantId },
        });
    ...
    const updated = await prisma.companies.update({
      ...
      data: {
        ...
        legalName: payload.legalName ?? company.legalName,
        taxId: payload.taxId ?? company.taxId,
...
export async function addCompanyContact(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const [company, contact] = await Promise.all([
      prisma.companies.findFirst({ where: { id: req.params.id, tenantId: auth.tenantId } }),
...
export async function removeCompanyContact(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const company = await prisma.companies.findFirst({ where: { id, tenantId: auth.tenantId } });
