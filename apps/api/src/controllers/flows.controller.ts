...
export const flowsController = {
  ...
  async create(req: Request, res: Response) {
    const { name, description, triggerType, triggerConfig, isActive } = req.body;
      ...
      data: {
        ...
        nodes: {
          create: {
            nodeType: 'START',
            label: 'Início',
            positionX: new Prisma.Decimal(250),
    ...
    logger.info({ flowId: flow.id, userId: req.user!.userId }, 'Flow created');
  ...
  async delete(req: Request, res: Response) {
    ...
    logger.info({ flowId: id, userId: req.user!.userId }, 'Flow deleted');
  ...
  async publish(req: Request, res: Response) {
    ...
    logger.info({ flowId: id, version: flow.version }, 'Flow published');
      ...
      data: {
        ...
        nodes: {
          create: original.nodes.map((node) => ({
            ...
            label: node.label,
            positionX: new Prisma.Decimal(Number(node.positionX ?? 0) + 50),
            positionY: new Prisma.Decimal(Number(node.positionY ?? 0) + 50),
            config: node.config ?? {},
          })),
        },
      },
  ...
  async validate(req: Request, res: Response) {
    ...
    const startNodes = flow.nodes?.filter((n: any) => (n.nodeType ?? n.type) === 'START') || [];
    ...
    flow.nodes?.forEach((node: any) => {
      ...
      if (!['TICKET', 'TYPEBOT'].includes(nodeType) && !nodesWithEdges.has(node.id) && nodeType !== 'START') {
        errors.push({
          field: `node:${node.id}`,
          message: `Nó "${node.label || nodeType}" não tem conexões de saída`
        });
      }
    });
