...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'DROP POLICY IF EXISTS "Users can view notification preferences" ON public.notification_preferences';
    EXECUTE 'CREATE POLICY "Users can view notification preferences" ON public.notification_preferences FOR SELECT USING (user_id = (SELECT auth.uid()))';

    EXECUTE 'DROP POLICY IF EXISTS "Users can update notification preferences" ON public.notification_preferences';
    EXECUTE 'CREATE POLICY "Users can update notification preferences" ON public.notification_preferences FOR UPDATE USING (user_id = (SELECT auth.uid()))';
  ELSE
    RAISE NOTICE 'Skipping notification_preferences policies because column user_id is missing.';
  END IF;
END;
$$;
